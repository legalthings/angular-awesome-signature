// Search for 'LegalThings' to find the modifications made to the build process
...
module.exports = function ( grunt ) {
  ...
  var taskConfig = {
    ...
    bump: {
      options: {
        ...
        pushTagOnly: true,
        pushTo: 'origin',
        bumpTag: true  // Base new version number on `git tag`, requires legalthings/grunt-bump fork.
      }
    },
    ...
    copy: {
      ...
      build_less: {
        files: [
          {
            src: ['<%= app_files.less %>', '<%= vendor_files.less %>'],
            dest: '<%= build_dir %>/',
            cwd: '.',
            expand: true
          }
    ...
    less: {
      build: {
        files: {
          '<%= build_dir %>/assets/<%= pkg.name %>.css': '<%= app_files.main_less %>'
        }
      },
      compile: {
        files: {
    ...
    delta: {
       ...
       * run our unit tests.
       * 
       * LegalThings: Skip linting (for now)
       */
      jssrc: {
        files: [ 
          '<%= app_files.js %>'
      ...
      assets: {
        ...
        tasks: [ 'copy:build_app_assets', 'copy:build_vendor_assets', 'copy:build_tpl' ]
      },
      
      /**
       * When the 'extra' files are changes, copy them.
       */
      extra: {
        files: [ '<%= app_files.extra %>' ],
        tasks: [ 'copy:build_extra' ]
      },
      ...
      /**
       * When the LESS files change, we need to copy and compile them.
       */
      less: {
        files: [ '<%= app_files.less %>', '<%= vendor_files.less %>' ],
        tasks: [ 'copy:build_less', 'less:build' ]
      },
   ...
   *
   * LegalThings: Disabled Karma as we didn't implement any tests.
   */
   ...
   *
   * LegalThings: Disabled linting (for now)
   * LegalThings: Disabled Karma as we didn't implement any tests
   */
